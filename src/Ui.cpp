...
Ui::Ui(std::shared_ptr<sf::RenderWindow> window, sf::Vector2f grid_size) {
    m_window = window;
...
void Ui::createStaticLeftSideArea() {
    ...
    hold_text.setPosition(sf::Vector2f(hold_text_x, 50));
    ...
    score_text.setPosition(sf::Vector2f(score_text_x, 400));
    ...
    level_text.setPosition(sf::Vector2f(level_text_x, 500));
    ...
    lines_text.setPosition(sf::Vector2f(lines_text_x, 600));
...
void Ui::createStaticRightSideArea() {
    ...
    next_text.setPosition(sf::Vector2f(lines_text_x, 50));
...
void Ui::createStaticGrid(sf::Vector2f grid_size) {
    for(int i = 1; i < grid_size.y; i++) {
        sf::RectangleShape line(sf::Vector2f(400, 0.5f));
        line.setPosition(sf::Vector2f(250, i*800/grid_size.y));
    ...
    for(int i = 1; i < grid_size.x; i++) {
        ...
        line.setPosition(sf::Vector2f(250+i*400/grid_size.x, 0));
        line.setFillColor(sf::Color(42, 42, 42));
...
void Ui::createGameArea() {
    ...
    m_game_area.setPosition(sf::Vector2f(250, 0));
}
...
void Ui::drawNextShapes(std::vector<TetrisShape>& next_shapes) {
    int offset = 0;
    ...
    for(TetrisShape& shape: next_shapes) {
        ...
        for(int i = 0; i < shape.m_cubes.size(); i++) {
            int x = 675+60 + offset_x - (x_bounds.x+x_bounds.y)*40 + (shape.m_offsets[i].x)*40;
            int y = offset + offset_y + 185/2 + 50 + (shape.m_offsets[i].y)*40;

            shape.m_cubes[i].setPosition(sf::Vector2f(x, y));
        }
        shape.draw(m_window);
        offset += 100;
    }
...
void Ui::drawHeldShape(TetrisShape& held_shape) {
    sf::Vector2i x_bounds = held_shape.calculateXBounds();
    ...
    for(int i = 0; i < held_shape.m_cubes.size(); i++) {
        int x = offset_x + 85 - (x_bounds.x+x_bounds.y)*40 + (held_shape.m_offsets[i].x)*40;
        int y = offset_y + 185/2 + 60 + (held_shape.m_offsets[i].y)*40;

        held_shape.m_cubes[i].setPosition(sf::Vector2f(x, y));
    }
    held_shape.draw(m_window);
}
...
void Ui::drawStats(int score, int level, int lines) {
    ...
    score_value.setPosition(sf::Vector2f(score_text_x, 445));
    ...
    level_value.setPosition(sf::Vector2f(level_text_x, 545));
    ...
    lines_value.setPosition(sf::Vector2f(lines_text_x, 645));
